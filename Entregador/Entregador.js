...
onSnapshot(pedidosRef, (snapshot) => {
  ...
  snapshot.forEach((documento) => {
    ...
    if (!distanciasCalculadas[id]) {
      ...
      distanciasCalculadas[id] = parseFloat(
        (Math.random() * limiteGerador + 0.5).toFixed(1)
      );
    }
    ...
    if (pedido.status === "Em Rota") {
      stats.emRota++;
      listaAndamento.innerHTML += criarCardAndamento(
        id,
        pedido,
        distancia,
        taxaGanha
      ...
      contadorDisponiveis++;
      listaPedidos.innerHTML += criarCardDisponivel(
        id,
        pedido,
        distancia,
        taxaGanha
  ...
  if (stats.emRota > 0) {
    ...
    if (contadorDisponiveis === 0) {
      listaPedidos.innerHTML =
        '<p style="color: #aaa; text-align:center;">Nenhum pedido disponível no momento.</p>';
    }
  }
        ...
        <div class="pedido-card">
            ...
            <div class="pedido-info">
                ...
                <div class="info-block" style="text-align: right;">
                    ...
                    <strong style="color: #d4af37;">R$ ${taxaGanha.toFixed(
                      2
                    )}</strong>
                </div>
            </div>
...
function criarCardAndamento(id, pedido, distancia, taxaGanha) {
  const shortId = "#" + id.substring(id.length - 4).toUpperCase();
  const valorPedido = pedido.resumo
    ? pedido.resumo.subtotal
    : pedido.total || 0;
...
function atualizarEstatisticas() {
  ...
  const media =
    stats.entregasHoje > 0 ? stats.somaDistancias / stats.entregasHoje : 0;
