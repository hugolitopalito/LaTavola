...
onSnapshot(mesasRef, (snapshot) => {
    ...
    mesasArray.forEach(mesa => {
        // Exibe apenas mesas ocupadas ou aguardando pagamento
        if (mesa.status !== 'Livre') {
            mesasOcupadasCount++;
            
            const cliente = mesa.cliente || "Sem nome";
            const total = mesa.total || 0;
            const itens = mesa.itens || []; // Garante que é um array, mesmo se estiver vazio
            
            // Verifica o status dos pratos para gerar a "tag"
            let badgeHTML = '';
            ...
            listaMesasGarcom.innerHTML += `
                <div class="mesa-item" onclick="window.location.href='./MesaGarcom/MesaGarcom.html?mesa=${mesa.id}'">
                    <div style="display: flex; gap: 20px; align-items: center;">
                        <strong style="color: #e74c3c; font-size: 1.2em;">#${mesa.numero}</strong>
                        <div>
                            <strong style="font-size: 1.1em;">Mesa ${mesa.numero}</strong><br>
                            <span style="color: #aaa; font-size: 0.85em;">${cliente} • ${mesa.pessoas} pessoas</span><br>
                            ${badgeHTML}
                        </div>
                    </div>
                    <div style="text-align: right;">
                        <strong style="color: #d4af37; font-size: 1.2em;">R$ ${total.toFixed(2)}</strong><br>
                        <span style="color: #aaa; font-size: 0.8em;">${itens.length} itens</span>
                    </div>
                </div>
            `;
        }
    ...
    if (mesasOcupadasCount === 0) {
        listaMesasGarcom.innerHTML = '<p style="color: #777;">Nenhuma mesa ocupada no momento.</p>';
    }
});