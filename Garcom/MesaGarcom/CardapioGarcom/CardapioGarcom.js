...
import { getFirestore, collection, getDocs, getDoc, doc, updateDoc, addDoc } from 'https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js';
...
// Consertando o botão de voltar dinamicamente
...
let dadosMesa = {};
let todosOsProdutos = [];
let carrinho = []; // Vai guardar { id, nome, preco, categoria, quantidade, cadeira }
let cadeiraAtual = 'Sem Cadeira';
let filtroAtual = 'Todos';

// 2. Carregar dados da Mesa (para saber quantas cadeiras desenhar)
...
function renderizarProdutos() {
    ...
    const filtrados = todosOsProdutos.filter(prod => 
        filtroAtual === 'Todos' || prod.categoria.toLowerCase() === filtroAtual.toLowerCase()
    );

    if (filtrados.length === 0) {
        listaProdutos.innerHTML = '<p style="color: #aaa;">Nenhum produto nesta categoria.</p>';
        return;
    }

    filtrados.forEach(produto => {
        // Quantidade DESTE produto NA cadeira atual
        const itemCart = carrinho.find(item => item.id === produto.id && item.cadeira === cadeiraAtual);
        ...
        listaProdutos.innerHTML += `
            <div class="produto-item">
                <div>
                    <strong style="font-size: 1.1em;">${produto.nome}</strong><br>
                    <span style="color: #aaa; font-size: 0.85em;">${produto.categoria}</span><br>
                    <strong style="color: #d4af37; font-size: 1.1em; display: inline-block; margin-top: 5px;">R$ ${produto.preco.toFixed(2)}</strong>
                </div>
                <div>
                    ${controlesHTML}
                </div>
...
function alterarQuantidade(produtoId, variacao) {
    ...
    let itemCarrinho = carrinho.find(item => item.id === produtoId && item.cadeira === cadeiraAtual);

    if (itemCarrinho) {
        itemCarrinho.quantidade += variacao;
        if (itemCarrinho.quantidade <= 0) {
            carrinho = carrinho.filter(item => !(item.id === produtoId && item.cadeira === cadeiraAtual));
        }
    } else if (variacao > 0) {
        carrinho.push({
            id: produtoBase.id,
...
function atualizarSidebar() {
    ...
    if (carrinho.length === 0) {
        itensCarrinho.innerHTML = '<p style="color: #777; text-align: center; margin: 30px 0;">Nenhum item selecionado</p>';
    ...
    carrinho.forEach(item => {
        ...
        const infoCadeira = item.cadeira !== 'Sem Cadeira' ? ` <span style="color:#aaa; font-size: 0.8em;">(${item.cadeira})</span>` : '';

        itensCarrinho.innerHTML += `
            <div class="item-carrinho">
                <div>
                    <strong>${item.nome}</strong>${infoCadeira}<br>
                    <span style="color: #aaa;">${item.quantidade}x R$ ${item.preco.toFixed(2)}</span>
                </div>
                <strong style="color: #d4af37;">R$ ${subtotal.toFixed(2)}</strong>
            </div>
        `;
    });
...
// 5. Salvar Pedido e Mandar pra Cozinha
btnConfirmarPedido.addEventListener('click', async () => {
    ...
    try {
        // Formata os itens para salvar (adiciona o status 'Pendente' neles)
        const itensParaSalvar = carrinho.map(item => ({
            ...item,
            status: "Pendente"
        }));

        const valorNovoPedido = carrinho.reduce((acc, i) => acc + (i.preco * i.quantidade), 0);

        // A. Atualiza a Comanda da Mesa (Adiciona na lista de itens da mesa e soma o valor)
        ...
        // B. Cria o documento na gaveta de "pedidos" para aparecer na tela da COZINHA
        await addDoc(collection(db, 'pedidos'), {
            data: new Date().toISOString(),
            status: "Pendente", // Para a cozinha ver na coluna 1
            mesaId: mesaId,
            numeroMesa: dadosMesa.numero,
            garcom: dadosMesa.garcom,
            // Não enviamos o objeto 'cliente', assim a cozinha sabe que é a la carte!
            itens: itensParaSalvar
        });

        alert("Pedido enviado para a cozinha com sucesso!");
        window.location.href = `.././MesaGarcom.html?mesa=${mesaId}`;
